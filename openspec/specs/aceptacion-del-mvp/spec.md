...
El MVP SHALL demostrar que un transportista identificado por el canal habilitado puede solicitar un turno, recibir una asignación persistida, consultar su estado e informar `EN_CAMINO`.

Un usuario interno autorizado SHALL poder continuar el ciclo normal definido en `turnos-ciclo-de-vida`, y el listado, el detalle y el dashboard MUST reflejar el estado vigente del mismo turno.
...
El MVP SHALL demostrar, con un turno distinto del utilizado en el recorrido principal, una cancelación autorizada desde un estado cancelable.
